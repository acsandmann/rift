...
- Symptom: after a relaunch, the next tab move snapped instantly instead of tweening. Three mechanisms combine to produce it:
  1. `window.frame_monotonic` is set to `target_frame` when an animation is *created*, and `AnimationManager::animate_layout` skips any later layout whose target `same_as` it (`src/actor/reactor/animation.rs:190`).

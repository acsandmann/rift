...
fn layout_commands_follow_active_display_space_across_active_displays() {
    ...
    for (wid, wsid, space, frame) in windows {
        ...
        reactor.window_manager.track_window_server_info(
...
fn user_space_window_server_destroyed_removes_window_when_window_server_is_gone() {
    ...
    assert!(!reactor.window_manager.contains_window(wid));
    assert_eq!(reactor.window_manager.tracked_window_id(wsid), None);
...
-> (Reactor, WindowId, WindowServerId, SpaceId, SpaceId, CGRect) {
    ...
    reactor.window_manager.set_window_server_space(wsid, Some(space2));
    reactor.window_manager.mark_window_visible(wsid);
    reactor.window_manager.insert_window(wid, WindowState {
        info: WindowInfo {
            is_standard: true,
...
fn hidden_window_can_move_to_another_native_space_without_staying_pinned_to_old_display() {
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager_mut()
            .assign_window_to_workspace(space1, wid, hidden_workspace)
    );
...
fn fullscreen_tracking_survives_until_ax_window_id_arrives() {
    ...
    assert!(
        reactor.window_manager.native_fullscreen_record_for_window(wid).is_none(),
...
fn mission_control_refresh_known_visible_fallback_does_not_restore_window_moved_to_other_space() {
    ...
    let _ = reactor
        .layout_manager
        .layout_engine
        .handle_virtual_workspace_command(space, &LayoutCommand::CreateWorkspace);
...
fn topology_change_clears_stale_pending_hide_target_before_next_workspace_layout() {
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager_mut()
            .assign_window_to_workspace(space, wid, hidden_workspace)
    );
    ...
    let hidden_target = reactor
        ...
        .calculate_layout_with_virtual_workspaces(
            ...
            Default::default(),
            |query_wid| {
                reactor.window_manager.window(query_wid).map(|window| window.frame_monotonic)
            },
            &[screen],
        )
        .into_iter()
...
fn authoritative_active_window_snapshot_removes_missing_window_from_active_layout() {
    ...
    reactor.window_manager.set_window_server_space(moved_wsid, Some(space));
    ...
    assert!(
        !reactor.window_manager.is_window_visible(moved_wsid),
...
fn authoritative_active_window_snapshot_reassigns_missing_window_to_inactive_space() {
    ...
    reactor
        .window_manager
        .set_window_server_space(retained_wsid, Some(active_space));
    reactor.window_manager.mark_window_visible(retained_wsid);
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager()
            .workspace_for_window(active_space, moved)
            .is_none(),
...
fn forwarded_space_state_does_not_clear_existing_fullscreen_tracks_when_snapshot_has_none() {
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager_mut()
            .assign_window_to_workspace(tracked_user_space, window_id, tracked_workspace)
    );
    let _ = reactor.window_manager.suspend_window_to_native_fullscreen(
        window_id,
...
fn non_active_workspace_windows_remain_hidden_even_if_frame_no_longer_matches_corner_geometry() {
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager_mut()
            .assign_window_to_workspace(space, wid, inactive_workspace)
    );
...
fn fullscreen_startup_applies_app_rules_to_hidden_user_space_windows() {
    ...
    reactor.window_manager.set_window_server_space(wsid, Some(user_space));
    ...
    assert_eq!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager()
            .workspace_for_window(user_space, wid),
...
fn fullscreen_startup_discovery_preserves_existing_hidden_assignment_without_app_rules() {
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager_mut()
            .assign_window_to_workspace(user_space, wid, secondary_workspace)
    );
...
fn unfullscreen_restores_window_tracking() {
    ...
    assert!(
        reactor.window_manager.native_fullscreen_record_for_window(window_id).is_none(),
...
fn wsid_rekey_preserves_non_default_workspace_without_app_rules() {
    ...
    assert!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager_mut()
            .assign_window_to_workspace(space, old_wid, secondary_workspace)
    );
    assert!(
        reactor
            .layout_manager
    ...
    assert_eq!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager()
            .workspace_for_window(space, new_wid),
        Some(secondary_workspace),
    ...
    assert_eq!(
        reactor
            ...
            .layout_engine
            .virtual_workspace_manager()
            .workspace_info_for_window_any(old_wid),
        None,

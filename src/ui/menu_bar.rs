...
use objc2_app_kit::{
    ...
    NSMenuItem, NSModalResponseOK, NSOpenPanel, NSRunningApplication, NSSavePanel, NSStatusBar,
    NSStatusItem, NSVariableStatusItemLength, NSView, NSWorkspace,
};
        ...
        let app_icon_group = if input.show_windows && input.show_app_icons {
            ...
            if icons.is_empty() {
                ...
                Some(AppIconGroupRenderData {
                    bg_rect: CGRect::new(
                        CGPoint::new(bg_x, bg_y),
                        CGSize::new(width, total_height),
                    ),
                    icons,
                })
            }
...
fn window_app_icon(window: &WindowData) -> Option<Retained<NSImage>> {
    ...
    window.app_name.as_deref().and_then(app_icon_for_application_name)
}
